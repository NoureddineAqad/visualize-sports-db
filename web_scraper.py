...
def scraper_menu(database):
    """
    Interactive web scraping menu
    ...
    print("="*60)
    print("\n1. Fetch Champions League teams")
    print("2. Fetch matchday results")
    ...
    if choice == "1":
        # Fetch teams
        print("\n" + "="*60)
        print("FETCH CHAMPIONS LEAGUE TEAMS")
        print("="*60)
        
        season = input("\nSeason (e.g., 2024-25, or Enter for current): ").strip()
        if not season:
            season = "2024-25"
        
        teams = scraper.fetch_champions_league_teams(season)
        
        # Let user verify
        verified_teams = scraper.verify_teams_interactive(teams)
        
        # Add to database
        if verified_teams:
            confirm = input(f"\nAdd {len(verified_teams)} teams to database? (y/n): ").strip().lower()
            if confirm == 'y':
                for team in verified_teams:
                    try:
                        database.add_team(team['name'], team['country'])
                    except ValueError as e:
                        print(f"⚠️  {e}")
                
                print(f"\n✅ Teams added to database!")
            else:
                print("❌ Teams not added")
        ...
        # Fetch results
        print("\n" + "="*60)
        print("FETCH MATCHDAY RESULTS")
        print("="*60)
        
        if not database.teams:
            print("❌ No teams in database. Add teams first!")
            input("\nPress Enter to continue...")
            return
        
        matchday = input("\nMatchday number: ").strip()
        if not matchday.isdigit():
            print("❌ Invalid matchday number")
            input("\nPress Enter to continue...")
            return
        
        matchday = int(matchday)
        
        results = scraper.fetch_matchday_results(matchday=matchday)
        ...
        # Let user verify
        verified_results = scraper.verify_results_interactive(results)
        
        # Add to database
        if verified_results:
            confirm = input(f"\nAdd {len(verified_results)} results to database? (y/n): ").strip().lower()
            if confirm == 'y':
                for result in verified_results:
                    try:
                        ...
                        for m in database.matches:
                            if (m.home_team == result['home_team'] and 
                                m.away_team == result['away_team'] and 
                                m.matchday == matchday):
                                match = m
                                break
                        ...
                        if not match:
                            ...
                            match = database.matches[-1]
                        
                        # Set result
                        if not match.played:
                            match.set_result(result['home_score'], result['away_score'])
                            database.teams[result['home_team']].update_stats(
                                result['home_score'], result['away_score']
                            )
                            database.teams[result['away_team']].update_stats(
