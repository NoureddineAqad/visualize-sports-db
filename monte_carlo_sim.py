...
def simulation_menu(database):
    """
    Interactive Monte Carlo simulation menu
    ...
    # Check if there are teams and matches
    if not database.teams:
        print("❌ No teams in database. Add teams first!")
        input("\nPress Enter to continue...")
        return
    
    if not database.matches:
        print("❌ No matches in database. Add matches first!")
        input("\nPress Enter to continue...")
        return
    
    # Check if there are unplayed matches
    unplayed = [m for m in database.matches if not m.played]
    if not unplayed:
        print("❌ All matches have been played. Nothing to simulate!")
        input("\nPress Enter to continue...")
        return
    
    print(f"\nTeams: {len(database.teams)}")
    print(f"Matches played: {len([m for m in database.matches if m.played])}")
    print(f"Matches remaining: {len(unplayed)}")
    
    # Ask for number of simulations
    while True:
        try:
            n_sims = input("\nNumber of simulations (default 10000, max 100000): ").strip()
            if not n_sims:
                n_sims = 10000
            else:
                n_sims = int(n_sims)
            ...
            if n_sims < 100:
                print("❌ Minimum 100 simulations")
                continue
            elif n_sims > 100000:
                print("❌ Maximum 100000 simulations")
                continue
    ...
    # Create simulator
    simulator = MonteCarloSimulator(database.teams, database.matches)
    
    # Run simulations
    print()
    results = simulator.run_simulations(n_sims)
    
    # Display results
    print()
    simulator.print_results(results)
    print()
    simulator.print_qualification_summary(results)
